...
function detailDrawer(id){
  ...
  drawer.innerHTML = `<header><div><small>${esc(r.id)}</small><h2>${esc(r.title)}</h2></div><button class="icon-btn drawer-close"><i class="ph ph-x"></i></button></header><div class="drawer-body"><section class="detail-status"><label>Estado</label><select id="status-select">${Object.entries(statusMeta).map(([key,x]) => `<option value="${key}" ${r.status === key ? "selected" : ""}>${x.label}</option>`).join("")}</select></section><section class="client-block"><span>${esc(r.customer_name).slice(0,1)}</span><div><small>CLIENTE</small><h3>${esc(r.customer_name)}</h3><p>${esc(r.customer_company || r.customer_email || "Cliente particular")}</p></div>${whatsapp ? `<a href="https://wa.me/598${esc(whatsapp)}" target="_blank" rel="noopener"><i class="ph ph-whatsapp-logo"></i></a>` : ""}</section><section class="detail-facts"><div><small>Cantidad</small><strong>${r.quantity} unidades</strong></div><div><small>Plazo</small><strong>${esc(r.deadline || "A definir")}</strong></div><div><small>Valor</small><strong>${money(r.amount)}</strong></div><div><small>Seña</small><strong>${money(r.deposit)}</strong></div></section><section class="drawer-delivery"><div><small class="eyebrow">ENTREGA</small><span class="status ${deliveryMetaItem.tone}"><i></i>${deliveryMetaItem.label}</span></div><h3><i class="ph ${delivery.method==="shipping"?"ph-truck":delivery.method==="pickup"?"ph-storefront":"ph-chat-circle-dots"}"></i>${esc(deliveryMethodLabel(delivery.method))}</h3><p>${esc(deliveryPlace||"Ubicación pendiente")}</p>${delivery.notes?`<small>${esc(delivery.notes)}</small>`:""}</section><section><small class="eyebrow">DETALLE DEL PEDIDO</small><p class="notes">${esc(r.notes || "Sin notas todavía.")}</p></section>${files.length ? `<section><small class="eyebrow">ARCHIVOS DE REFERENCIA</small><div class="attachment-list">${files.map(file => `<button data-attachment="${esc(file.storage_path)}"><i class="ph ph-paperclip"></i><span><strong>${esc(file.original_name)}</strong><small>${file.size_bytes ? `${Math.ceil(file.size_bytes / 1024)} KB` : "Archivo adjunto"}</small></span><i class="ph ph-arrow-square-out"></i></button>`).join("")}</div></section>` : ""}${job ? `<section><div class="inline-head"><small class="eyebrow">FICHA DE PRODUCCIÓN</small><button id="edit-job">Editar ficha</button></div><div class="production-facts"><span><b>Material</b>${esc(job.material || "A definir")}</span><span><b>Color</b>${esc(job.color || "A definir")}</span><span><b>Peso</b>${job.grams_estimated ? `${job.grams_estimated} g` : "A definir"}</span><span><b>Máquina</b>${esc(job.printer_name || "A definir")}</span></div></section>` : ""}<section><small class="eyebrow">ORIGEN</small><p class="notes">${esc(r.source || "manual")} · creada ${shortDate(r.created_at)}</p></section></div><footer><button class="secondary" id="drawer-note"><i class="ph ph-note-pencil"></i>Agregar nota</button><button class="primary" id="drawer-edit"><i class="ph ph-pencil-simple"></i>Editar</button></footer>`;

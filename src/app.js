...
const statusPill = id => { const meta = statusMeta[id] || {label:id, tone:"gray"}; return `<span class="status ${meta.tone}">${esc(meta.label)}</span>`; };
...
function productModal(item=null){
  ...
  openModal({title:item ? "Editar producto" : "Nuevo producto", eyebrow:"CATÁLOGO", description:"Completá la información y ajustá la imagen exactamente como querés que aparezca en el catálogo.", wide:true, modalClass:"product-modal", content:`<div class="product-editor">
    <section class="product-editor-image">
      ...
      <div class="crop-stage"><div class="editor-preview ${image ? "has-image" : ""}" id="product-image-preview" tabindex="0" aria-label="Editor de encuadre. Arrastrá para mover y usá la rueda o pellizcá para ampliar.">${image ? `<img src="${esc(image)}" alt="" draggable="false" style="${imageFrameStyle(item)}">` : `<img src="${logo}" alt="ATRY" draggable="false"><span>Subí una imagen para comenzar</span>`}<span class="crop-grid" aria-hidden="true"></span><span class="crop-corner top-left" aria-hidden="true"></span><span class="crop-corner top-right" aria-hidden="true"></span><span class="crop-corner bottom-left" aria-hidden="true"></span><span class="crop-corner bottom-right" aria-hidden="true"></span><button type="button" class="crop-hint crop-lock-toggle"><i class="ph ph-hand-grabbing"></i><span>Arrastrá para mover</span></button></div></div>
...
function bindProductImagePreview(){
  ...
  const mobileCrop = window.matchMedia("(max-width:760px)"); const cropToggle=preview.querySelector(".crop-lock-toggle"); const cropToggleLabel=cropToggle?.querySelector("span"); const cropToggleIcon=cropToggle?.querySelector("i");
  const setCropLocked = locked => { preview.classList.toggle("crop-locked", mobileCrop.matches && locked); preview.classList.toggle("crop-editing", mobileCrop.matches && !locked); preview.setAttribute("aria-label", mobileCrop.matches && locked ? "Vista previa de la imagen. Tocá Ajustar imagen para modificar el encuadre." : "Editor de encuadre. Arrastrá para mover y pellizcá para ampliar."); if(cropToggleLabel) cropToggleLabel.textContent=mobileCrop.matches ? (locked ? "Ajustar imagen" : "Listo") : "Arrastrá para mover"; if(cropToggleIcon) cropToggleIcon.className=`ph ${mobileCrop.matches ? (locked ? "ph-sliders-horizontal" : "ph-check") : "ph-hand-grabbing"}`; };
  setCropLocked(true); mobileCrop.addEventListener?.("change",()=>setCropLocked(true)); cropToggle?.addEventListener("click", event => { event.stopPropagation(); if(!mobileCrop.matches) return; setCropLocked(!preview.classList.contains("crop-locked")); });
  const pointers = new Map(); let lastPoint=null; let pinchStart=null;
  preview.addEventListener("pointerdown", event => { if(event.target.closest(".crop-lock-toggle") || !preview.classList.contains("has-image") || (mobileCrop.matches && preview.classList.contains("crop-locked"))) return; preview.setPointerCapture(event.pointerId); pointers.set(event.pointerId,{x:event.clientX,y:event.clientY}); lastPoint={x:event.clientX,y:event.clientY}; pinchStart=null; preview.classList.add("dragging"); });
